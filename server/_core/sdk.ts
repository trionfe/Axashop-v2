...
// User type inline (no drizzle/MySQL needed)
...
class SDKServer {
  ...
  async authenticateRequest(req: any): Promise<User> {
    ...
    // Bypass DB for admin-session as requested
    ...
    // Only update lastSignedIn if DB is available
    try {
      await db.upsertUser({
        openId: user.openId,
        lastSignedIn: signedInAt,
      ...
      console.warn("[Auth] Skipping user update: DB not available");
    }
