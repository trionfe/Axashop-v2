...
export default function ProductPage() {
  ...
  const isSupabaseGroup = Boolean(productId?.startsWith("sg-"));
  const supabaseGroupId = isSupabaseGroup ? productId.replace("sg-", "") : null;

  useEffect(() => {
    import("@/lib/products").then(m => m.getProductsAsync().then(setAllProducts));
  }, []);
  ...
  const variants: any[] = isSupabaseGroup && supabaseGroup
    ? (supabaseGroup.options || []).map((o: any) => ({
        id: o.id || `sg-opt-${o.name}`,
        nameKey: o.name,
        descKey: o.name,
        ...
        image: o.image || supabaseGroup.image || "",
        columnId: supabaseGroup.category,
      }))
    : group
      ? allProducts.filter((p:any) => group.ids.includes(p.id))
  ...
  useEffect(() => {
    if (!isSupabaseGroup && !group && !singleProduct && allProducts.length > 0) navigate("/");
  ...
  const heroImage = selectedVariant.image || (isSupabaseGroup ? supabaseGroup?.image : group?.image) || "";
  const pageTitle = isSupabaseGroup ? (supabaseGroup?.label || "") : group ? group.label : (t[selectedVariant.nameKey] || selectedVariant.nameKey);
  const pageCategory = isSupabaseGroup ? (supabaseGroup?.category || "") : group ? group.category : selectedVariant.columnId;
      ...
      <div className="relative z-10 container py-10 max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-20 items-start">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="lg:sticky lg:top-24 space-y-4">
            <div className="aspect-[16/9] rounded-[2rem] overflow-hidden bg-white/[0.02] border border-white/[0.06] relative">
                    ...
                    return (
                      <button key={v.id} onClick={() => setSelectedVariantId(v.id)}
                        ...
                        <div className="aspect-[16/9] overflow-hidden relative">
                          <img src={SOCIAL_IMAGES[v.id] || v.image || group?.image || supabaseGroup?.image || ""} alt={isSupabaseGroup ? v.nameKey : (t[v.nameKey] || v.nameKey)} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                          <div className="absolute inset-0 bg-gradient-to-t from-[#030711] via-transparent to-transparent" />
                        </div>
                        <div className="p-3 bg-white/[0.02]">
                          <p className="text-xs font-black text-white leading-tight line-clamp-2 mb-1">{isSupabaseGroup ? v.nameKey : (t[v.nameKey] || v.nameKey)}</p>
